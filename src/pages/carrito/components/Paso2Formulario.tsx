...
const Paso2Formulario = ({ formData, usuario, onPrev, onNext }: Props) => {
  ...
  const inputClass = "bg-[#1a1a1a] border border-neutral-800 rounded-md px-4 py-3 text-sm focus:outline-none focus:border-[#8b5cf6] transition-colors w-full text-white";
  const inputErrorClass = "bg-red-500/5 border border-red-500 rounded-md px-4 py-3 text-sm focus:outline-none transition-colors w-full text-white";
  const disabledInputClass = "bg-[#111111] border border-neutral-800 text-neutral-500 rounded-md px-4 py-3 text-sm cursor-not-allowed w-full";
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-4xl mx-auto w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-semibold text-neutral-300">País de Residencia</label>
        ...
        <div className="flex flex-col gap-2 md:col-span-2 relative">
          ...
          <div className="flex items-center">
            ...
            <button type="button" className="absolute right-2 text-xs bg-[#2a2a2a] hover:bg-[#333] text-white px-3 py-1.5 rounded transition-colors flex items-center gap-1">
        ...
        {esArgentina && (
          <div className="flex flex-col gap-3 md:col-span-2 p-5 bg-[#8b5cf6]/5 border border-[#8b5cf6]/30 rounded-xl">
            <label className="text-xs font-semibold text-[#8b5cf6]">Documento para Facturación AFIP</label>
            <div className="flex gap-2 p-1 bg-[#1a1a1a] rounded-lg border border-neutral-800 w-full md:w-1/2">
              <button type="button" onClick={() => cambiarTipoDocumento('DNI')} className={`flex-1 py-2 rounded-md text-xs font-bold transition-all ${tipoDocActual === 'DNI' ? 'bg-[#8b5cf6] text-white shadow-md' : 'text-neutral-400 hover:text-white'}`}>DNI </button>
              <button
                type="button"
                onClick={() => cambiarTipoDocumento('CUIT')}
                className={`flex-1 py-2 rounded-md text-xs font-bold transition-all ${tipoDocActual === 'CUIT' ? 'bg-[#8b5cf6] text-white shadow-md' : 'text-neutral-400 hover:text-white'}`}
              >
                CUIT / CUIL
              </button>
            </div>
                ...
                {...register('documentoIdentidad', { 
                  ...
                  validate: (value) => {
                    ...
                    if (tipoDocActual === 'CUIT') {
                      ...
                      return calculado === digitoVerificador || 'El CUIT/CUIL ingresado es inválido (Error matemático).';
                    }
                  }
                })}
              />
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-semibold text-neutral-300">Número de teléfono</label>
          <input 
            type="text" 
        ...
        <div className="flex flex-col gap-2 md:col-span-2">
          <label className="text-xs font-semibold text-neutral-300">Dirección Completa</label>
          <input type="text" placeholder="Ej: Calle Falsa 123, Piso 2..." className={inputClass} {...register('direccion')} />
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-xs font-semibold text-neutral-300">Provincia / Estado</label>
        ...
        <div className="flex flex-col gap-2 md:col-span-2">
          <label className="text-xs font-semibold text-neutral-300">Código Postal</label>
          <input type="text" className={inputClass} {...register('codigoPostal')} />
        </div>
      </div>

      <div className="flex justify-center items-center gap-4 pt-12">
        <button type="button" onClick={onPrev} className="w-40 bg-transparent border border-neutral-700 hover:border-neutral-500 text-white font-semibold py-3 rounded-lg transition-colors">
          Anterior
        </button>
        <button type="submit" className="w-40 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-semibold py-3 rounded-lg transition-colors">
          Siguiente
        </button>
      </div>
    </form>
